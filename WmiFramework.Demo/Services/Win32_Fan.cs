...
namespace WmiFramework.Demo.Services
{
    ...
    /// </summary>
    public class Win32_Fan : WMIService<Entites.Win32_FanEntity>
    {
        public Win32_Fan(ConnectionOptions options, string address) : base(options, address) { }
        /// <summary>
        ...
        /// <param name="Time"></param>
        /// <returns></returns>
        public uint SetPowerState(Entites.Win32_FanEntity entity, ushort PowerState, DateTime Time)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public uint Reset(Entites.Win32_FanEntity entity)
        {
            var classObj = GetEntityWmiObjecj(entity);
            var inParams = classObj.GetMethodParameters("Reset");
            var output = classObj.InvokeMethod("Reset", inParams, null);
        ...
        /// <summary>
        /// 请求将风扇速度设置为方法的输入参数中指定的值。如果请求被成功执行，则返回值应当为 0；如果不支持请求，则返回值为 1；如果出现错误，则返回其他值。 
        /// </summary>
        /// <param name="DesiredSpeed"></param>
        /// <returns></returns>
        public uint SetSpeed(ulong DesiredSpeed)
        {
            using (var classObj = GetClass())
            {
                var inParams = classObj.GetMethodParameters("SetSpeed");
                inParams["DesiredSpeed"] = NetValueToWmi(CimType.UInt64, DesiredSpeed);
                var output = classObj.InvokeMethod("SetSpeed", inParams, null);
                return (uint)WmiValueToNet(CimType.UInt32, output["ReturnValue"]);
            }
        }
        /// <summary>
        ...
        /// <param name="DesiredSpeed"></param>
        /// <returns></returns>
        public uint SetSpeed(Entites.Win32_FanEntity entity, ulong DesiredSpeed)
        {
            var classObj = GetEntityWmiObjecj(entity);
            var inParams = classObj.GetMethodParameters("SetSpeed");
            inParams["DesiredSpeed"] = NetValueToWmi(CimType.UInt64, DesiredSpeed);
